 ...
 */
class AutenticacionCorreo(
    contexto: Context,
    private val baseRemota: BaseRemota
) {
    ...
    private suspend fun com.google.firebase.auth.FirebaseUser.aUsuario(): Usuario {
        val correo = email.orEmpty()
        val nombreVisible = displayName?.takeIf { it.isNotBlank() }
            ?: correo.substringBefore('@').ifBlank { "Operador" }
        val rolRemoto = baseRemota.obtenerOCrearRol(uid, correo, nombreVisible)
        return Usuario(
            id = uid,
            usuario = correo,
